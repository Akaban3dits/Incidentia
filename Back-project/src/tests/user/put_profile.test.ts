...
describe("PUT /api/users/:userId/profile", () => {
  it("✅ actualiza teléfono y departamento", async () => {
    ...
    const dep = await withAuth(
      request(app).post("/api/departments").send({ name: "Sistemas" }),
      token
    );
    ...
    const res = await withAuth(
      request(app).put(`/api/users/${created.body.user_id}/profile`).send({
  ...
  it("✅ permite limpiar department_id con null", async () => {
    ...
    const dep = await withAuth(
      request(app).post("/api/departments").send({ name: "Calidad" }),
      token
    );

    const u = await withAuth(
      request(app).post("/api/users").send({
        first_name: "Ana",
        last_name: "Doe",
    ...
    await withAuth(
      request(app).put(`/api/users/${u.body.user_id}/profile`).send({
        department_id: dep.body.id,
      }),
      token
    );
